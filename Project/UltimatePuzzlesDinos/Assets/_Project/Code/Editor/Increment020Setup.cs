...
{
    public static class Increment020Setup
    {
        ...
        public static void Run()
        {
            ...
            MosaicBoardController mosaicBoard =
                standardBoard.GetComponent
            ...
            SerializedObject mosaicSerialized =
                new(mosaicBoard);

            mosaicSerialized.FindProperty("layoutSource")
                .objectReferenceValue = standardBoard;

            mosaicSerialized.ApplyModifiedPropertiesWithoutUndo();

            SerializedObject screenSerialized =
                new(gameplayScreen);

            screenSerialized.FindProperty(
                    "mosaicBoardController")
                .objectReferenceValue = mosaicBoard;

            screenSerialized.ApplyModifiedPropertiesWithoutUndo();

            EditorUtility.SetDirty(mosaicBoard);
            ...
            Debug.Log(
                "[Ultimate Puzzles Dinos] Increment 020 installed. " +
                "Mosaic is now playable.");

            EditorUtility.DisplayDialog(
                "Ultimate Puzzles Dinos",
                "Increment 020 completado.\n\n" +
                "El modo Mosaic ya es jugable.",
                "OK");
        }
        ...
        public static void Validate()
        {
            ...
            else
            {
                ...
                if (!roots.Any(root =>
                    root.GetComponentInChildren
                        <MosaicBoardController>(true) != null))
                {
                    problems.Add(
                        "Gameplay no contiene MosaicBoardController.");
                }
            ...
            if (problems.Count == 0)
            {
                Debug.Log(
                    "[Ultimate Puzzles Dinos] " +
                    "Increment 020 validation passed.");

                EditorUtility.DisplayDialog(
                    "Validación correcta",
                    "Increment 020 instalado correctamente.",
                    "OK");
                return;
            }
            ...
            Debug.LogError(
                "[Ultimate Puzzles Dinos] " +
                "Increment 020 validation failed:\n- " +
                report);
